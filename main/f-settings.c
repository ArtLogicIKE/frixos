...
#include "build_time.h" // BUILD_TIMESTAMP — regenerated on every build by CMake
...
esp_err_t status_api_handler(httpd_req_t *req)
{
    ...
    // BUILD_TIMESTAMP comes from build_time.h, regenerated by a CMake custom
    // target on every build (see main/CMakeLists.txt + gen_build_time.cmake).
    // Unlike __DATE__/__TIME__ or the app descriptor, this refreshes on ANY
    // recompile, not just when this file changes.
    cJSON_AddStringToObject(root, "compile_time", BUILD_TIMESTAMP);
