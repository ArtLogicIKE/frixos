...
void init_char_width_cache(const lv_font_t *font)
{
  ...
  for (int i = 0; i < 128; i++)
  {
    // O(1) direct-mapped hash lookup using bitwise AND instead of modulo (CACHE_SIZE is 512)
...
static uint8_t get_cached_char_width(uint32_t code_point, const lv_font_t *font, const char *text, int text_pos)
{
  ...
  // O(1) Lookup: Use bitwise AND for direct indexing (CACHE_SIZE is 512). Handle collisions by simple replacement.
                              ...
                              lv_obj_t *label_obj, const lv_font_t *font)
{
  ...
  // Optimization: only call lv_label_set_text if the text has changed
  ...
  // Optimization: multi-label safe caching by tracking label_obj and font
  static lv_obj_t *last_label_obj = NULL;
  static const lv_font_t *last_cached_font = NULL;
  static int32_t last_x = -1, last_y = -1, last_width = -1;

  // Invalidate cache if label object or font changes
  if (last_label_obj != label_obj || last_cached_font != font)
  {
    last_label_obj = label_obj;
    last_cached_font = font;
    last_x = -1;
    last_y = -1;
    last_width = -1;
  }
