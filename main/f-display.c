...
void set_scroll_message(const char *msg)
{
  ...
  const char *curr_text = lv_label_get_text(label_msg);
  if (curr_text == NULL || strcmp(curr_text, msg) != 0)
  {
    lv_label_set_text(label_msg, msg);
  }
  // Apply font before measurement
  ...
  { // centered
    ...
    ESP_LOG_WEB(ESP_LOG_INFO, TAG, "set_scroll_message: centered (%d px)", label_size);
    label_max_pos = 0;
    label_scroll_pos = 0;
  }
  lvgl_port_unlock();
...
void startup_display(void)
{
  ...
  // Create clipped container for scrolling message
  ...
  lv_obj_set_size(msg_container, MSG_WIDTH, 14); // Standard height for our fonts (max 11px)
