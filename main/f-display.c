...
void set_scroll_message(const char *msg)
{
  ...
  // Bolt Optimization: Set the full message text once.
  // We now scroll by shifting the label's X position instead of re-rendering substrings.
  ...
  { // scrolling, left aligned
    ...
    label_scroll_pos = -MSG_WIDTH; // Start scrolling from the right edge
    lv_obj_set_pos(label_msg, MSG_WIDTH, 0);
  }
  else
  { // centered
...
void startup_display(void)
{
  ...
  // This allows us to simply update the label's X position instead of re-rendering substrings,
  // significantly reducing CPU usage in the high-frequency display loop.
  ...
  lv_obj_set_size(msg_container, MSG_WIDTH, get_selected_font_height(eeprom_msg_font));
  lv_obj_set_pos(msg_container, (eeprom_ofs_x > 6 ? eeprom_ofs_x - 6 : eeprom_ofs_x), eeprom_ofs_y + label_msg_ofs_y);
  ...
  lv_obj_clear_flag(msg_container, LV_OBJ_FLAG_SCROLLABLE); // Ensure no automatic scrolling
...
void display_changed(void)
{
  ...
  // Note: Maintain the minor 6-pixel offset adjustment used in the previous implementation
  lv_obj_set_pos(msg_container, (eeprom_ofs_x > 6 ? eeprom_ofs_x - 6 : eeprom_ofs_x), eeprom_ofs_y + label_msg_ofs_y);
  lv_obj_set_height(msg_container, get_selected_font_height(eeprom_msg_font));
...
void display_task(void *pvParameters)
{
  ...
  while (1)
  {
    ...
    { // scrolling, left aligned
      ...
      // Bolt Optimization: Instead of re-rendering substrings, simply shift the label's X position.
      // This offloads the heavy work to LVGL's rendering engine and skips expensive logic in display_task.
