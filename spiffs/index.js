...
// Optimization: Initialize as empty to ensure first translate() call (even if English) updates DOM
let currentLanguage = '';
...
async function translate(lang) {
    ...
    document.querySelectorAll('[data-i18n], [data-i18n-placeholder], [data-i18n-aria-label]').forEach(element => {
        // Optimization: Use WeakMap to cache metadata and last-seen values to avoid redundant dataset reads and DOM writes
        let metadata = i18nCache.get(element);
        if (!metadata) {
            metadata = {
                k: element.dataset.i18n,
                pk: element.dataset.i18nPlaceholder,
                ak: element.dataset.i18nAriaLabel,
                v: '',
                pv: '',
                av: ''
            };
        ...
        if (metadata.k) {
            const translation = getNestedTranslation(trans, metadata.k);
            if (translation && metadata.v !== translation) {
                element.innerHTML = translation;
                metadata.v = translation;
            }
        }

        if (metadata.pk) {
            const translation = getNestedTranslation(trans, metadata.pk);
            if (translation && metadata.pv !== translation) {
                element.placeholder = translation;
                metadata.pv = translation;
            }
        }

        if (metadata.ak) {
            const translation = getNestedTranslation(trans, metadata.ak);
            if (translation && metadata.av !== translation) {
                element.setAttribute('aria-label', translation);
                metadata.av = translation;
            }
        }
    });

    // Optimization: Hoist invariant lookups outside the loops
