...
async function translate(lang) {
    ...
    // Update font sample box ARIA labels after language change
    ...
    fontSampleBoxesCache.forEach(box => {
        ...
        const fontName = box.querySelector('.font-sample-name');
        if (fontName) {
            box.setAttribute('aria-label', `${selectLabel} ${fontName.textContent} font`);
        }
    });
    ...
    if (!window.advancedEventListenersSet) {
        ...
        document.querySelectorAll('.font-sample-box').forEach(box => {
            const selectFont = () => {
                const fontName = box.querySelector('.font-sample-name');
                if (!fontName) return;
                const fontValue = fontName.textContent.toLowerCase();
