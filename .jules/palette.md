...
## 2026-05-28 - [Interactive Preview Selection Pattern]
**Learning:** Static previews (like font samples) are often perceived as interactive by users. Transforming them into accessible buttons that synchronize with existing form controls (dropdowns) creates a more direct and intuitive UI. Localized ARIA labels can be efficiently generated by combining a generic "Select" translation with the element's own descriptive text.
**Action:** Identify static preview elements that represent selectable options and enhance them with role="button", keyboard support, and state synchronization to improve UX discoverability.
