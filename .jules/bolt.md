...
## 2026-05-24 - Refined Metadata Caching for Translation System
**Learning:** While lazy-loading elements reduced DOM queries, repetitive access to element properties like `dataset` (which uses a Proxy) and `getAttribute`, as well as repeated DOM traversals (`previousElementSibling`), still added measurable script execution overhead during language switches. In a Single Page Application on an embedded device, these micro-overheads accumulate across hundreds of translatable elements.
**Action:** Extend the translation cache to store pre-parsed metadata (keys, shorthand attributes) and DOM relations alongside the element references. Map NodeLists into plain object arrays during the first translation cycle to ensure subsequent switches use O(1) property lookups. Hoist loop-invariant translation lookups (`getNestedTranslation`) to eliminate redundant nested object traversals.
