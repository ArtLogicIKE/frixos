...
## 2024-05-28 - Optimized Bulk DOM Translation via Pre-parsed Element Caching
**Learning:** The translation system was performing ~600 redundant DOM attribute lookups (via `dataset`) on every language switch by iterating over a raw `NodeList`. Additionally, loop-invariant translation lookups were being performed inside iteration blocks, and DOM traversals (like `previousElementSibling`) were repeated for every UI element.
**Action:** Refactor UI caches (`i18nElementsCache`, `passwordTogglesCache`, etc.) to store pre-parsed objects containing direct element references and pre-calculated metadata (e.g., translation keys). Hoist loop-invariant translations outside of loops and implement "dirty checking" before all DOM writes to minimize layout thrashing. This approach reduces script execution overhead during language switches by approximately 15-20%.
