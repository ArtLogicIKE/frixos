...
## 2026-05-30 - Optimized Translation System with Object-Based Caching
**Learning:** While NodeList caching reduces global DOM queries, it still leaves redundant attribute lookups (e.g., `dataset`) inside hot loops. Accessing DOM `dataset` properties is significantly more expensive than direct object property access. Furthermore, caching dynamic `textContent` inside these structures for ARIA labels leads to stale UI state if the underlying text changes.
**Action:** Refactor DOM caches to store arrays of objects with pre-parsed keys (`{ el, i18nKey, ... }`). Implement explicit dirty checking (comparing new value vs `textContent`) to avoid layout thrashing. Always re-fetch `textContent` for dynamic labels (like token codes) inside the loop while keeping the element reference cached to ensure functional correctness.
