...
## 2026-05-24 - Refined Object-Based Caching for Translation System
**Learning:** While lazy caching of `NodeList` results reduces DOM query overhead, it still leaves significant script execution time due to redundant `dataset` attribute lookups and relationship traversals (e.g., `previousElementSibling`) within the translation loops. In a UI with ~200 translatable elements, these O(N) operations trigger hundreds of redundant browser API calls on every language switch.
**Action:** Transition from raw element caches to object arrays that store pre-parsed attributes and related DOM references. Hoist loop-invariant translation lookups (e.g., common labels) outside of iteration blocks. Implement strict dirty-checking (equality guards) for all DOM updates, including ARIA labels and page titles, to minimize layout thrashing and paint cycles.
