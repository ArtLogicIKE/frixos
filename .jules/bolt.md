...
## 2026-05-11 - Optimized Frontend DOM Updates and Translation Logic
**Learning:** In resource-constrained environments like the ESP32, repeated DOM writes (even with identical content) trigger expensive layout and paint calculations in the browser. Furthermore, redundant object allocations and repeated property lookups (like translation keys) in high-frequency loops create significant CPU overhead and garbage collection pressure.
**Action:** Implement a `setSafe` helper for all DOM updates to perform dirty-checking before mutation. Hoist static constants (byte sizes, moon phases) and invariant translation lookups out of loops and high-frequency functions. This pattern ensures that DOM interaction only occurs on actual state changes, preserving CPU cycles for smooth UI responsiveness.
