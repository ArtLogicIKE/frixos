...
## 2026-05-20 - Refined JavaScript DOM Interaction and Object Merging
**Learning:** Even with lazy-initialized element caches, performing ~600 redundant 'dataset' lookups and 'getAttribute' calls during every language switch adds measurable execution overhead. Furthermore, building translatable labels for dynamic elements (like tokens) by caching their initial 'textContent' leads to stale ARIA labels when values change.
**Action:** Transition caches from raw NodeLists to object arrays containing both element references and pre-parsed metadata (keys, associated inputs). Implement dirty checking (equality guards) before all DOM writes (innerHTML, textContent, attributes) to minimize layout thrashing. Avoid caching dynamic text content; instead, re-read it during the translation loop while maintaining the O(1) element lookup. Use 'Object.assign' for settings merging to replace slower manual iteration.
