...
## 2026-05-22 - Image Asset Caching in Layout Editor
**Learning:** High-frequency rendering (e.g., during element dragging at 60fps) in the Layout Editor was triggering redundant `new Image()` probe creations and asynchronous style applications. This resulted in visual flicker as elements would appear invisible for a few frames while the image was re-probing on every move event. Additionally, shared assets (sprite sheets) could lead to broken rendering if multiple elements requested the same loading image.
**Action:** Implement a module-level `_screenImageCache` with a `queue` for concurrent requests. Cache image dimensions and success/failure state to allow synchronous style application for already-loaded assets. This eliminates flicker during drag interactions and significantly reduces GC pressure from short-lived `Image` objects.
